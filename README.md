...
* **记忆系统**：总结模式（经典）、向量模式（本地嵌入模型，嵌入计算在设备本地完成，数据不出设备）与时间线模式（故事时钟、事实时间锚定、时间线摘要与图谱），支持总结记忆一键迁移为向量，并提供事实层抽取与记忆召回。
...
* **时间线记忆**：引入故事时钟与时间解析（「昨晚 / 三日后 / 周五 / 16:00」→ 绝对时间锚点），事实 Schema v2 带时间锚定与差分写入，时间线摘要注入上下文，并提供时间线图谱与时钟 UI。
